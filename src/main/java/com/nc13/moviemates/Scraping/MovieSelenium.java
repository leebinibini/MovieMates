...
public class MovieSelenium {
    ...
    public List<MovieDomain> crawlMovies(String url) {
        WebDriver driver = new ChromeDriver();
        driver.get(url);

        List<MovieDomain> movies = new ArrayList<>();
        ...
        for (int i = 0; i < movieLinks.size(); i++) {
            ...
            // MovieDomain 객체 생성
            MovieDomain movieDomain = new MovieDomain(title, releaseDate, runningTime, information, genre, director);
